...
这一部分将简明扼要的介绍主要模块的设计考量。更详细的设计细节可见下一个章节“[主要模块实现](#modimpl)”。
...
译码器，将16位指令译码为5位指令编号Op，3个寄存器地址reg1、reg2、reg3和16位立即数imm。在译码器中根据Op进行了立即数的扩展。3个寄存器地址中reg1、reg2为下一步将要读取的寄存器、reg3为将要写入的寄存器。

### MemoryTop

MemoryTop是总线上负责所有外围设备的通信的一个模块，相当于实际计算机中连接南北桥的芯片，目的是向CPU封装所有的外围逻辑，简化CPU内部设计。

其工作可描述为：CPU从MemoryTop获取指令和数据，写入数据也需要通过MemoryTop。我们的MemoryTop负责管理RAM、FLASH、串口、PS2、VGA。
...
寄存器堆，负责所有寄存器，寄存器编址4位。具体可参见[寄存器编址分配表](#regfileaddr)。所有寄存器均是下降沿写入。同时具有调试输出接口。
...
所有指令中涉及到的T的写入分为2种，不等于和大于，所以我们通过alu做减法，不等于即为alu的输出不为0，大于即为alu输出符号位为0。于是我们就可以直接根据控制信号Ttype,Twrite及ALU的输出计算出T寄存器的值输出。简化内部逻辑。

### VGA\_top

VGA管理模块。具有显存。外部可以通过指定地址和内部约定编码，更改显示内容。

## 软件

### assembler(thcoas.py)

我们在武祥晋同学(2011011278)的python实现的汇编翻译器的基础上进行了更改，实现了对我们自己定义的指令集的编译工作。
...
通过Case语句判断高五位的值得到指令类型（5位指令编号Op），根据指令类型Op的不同决定在后续流程中将要读写的寄存器和立即数扩展的方式（每一条指令的执行细节可参见指令文档`instruction.xlsx`），输出下一步即将要读写的寄存器编址reg1（读）、reg2（读）、reg3（写）（没有使用寄存器则赋为零寄存器地址），以及扩展后的16位立即数imm。
...
该模块在调试时被编写成可以用来即时的十六进制显示各种输入数据，以`VGA_play.vhd`
