...
具体使用方法：`python binToHex.py a.s`。汇编器会将MIPS汇编代码a.s编译输出a.s.bin的二进制文件，并在命令行上输出丰富的调试信息（二进制、十六进制代码、代码原文行号、翻译后的代码地址号），从而极大的方便了后续的调试。

![assembler截图][assembler.jpg]
...
1. 正确连线， 将开关第5位拨到1(自动时钟)  

2. 打开FlashAndRam，将代码烧到Ram的0x4000处

	![FlashandRam使用][flashandram.jpg]
